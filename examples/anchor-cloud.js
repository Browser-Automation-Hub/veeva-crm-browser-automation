/**
 * anchor-cloud.js — AnchorBrowser cloud example for Veeva CRM (Veeva Commercial Cloud)
 *
...
const { login_veeva, create_call_report } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Veeva CRM (Veeva Commercial Cloud) automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_veeva(page, {});
      log('Action 1 result:', data1);

      const data2 = await create_call_report(page, {});
