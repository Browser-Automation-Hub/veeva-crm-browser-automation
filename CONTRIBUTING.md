# Contributing to Veeva CRM (Veeva Commercial Cloud) Browser Automation
...
- **Broken selectors**: Veeva CRM (Veeva Commercial Cloud) updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Veeva CRM (Veeva Commercial Cloud) instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.VEEVA_CRM_URL}/path/to/section`);
...
When you find working selectors for Veeva CRM (Veeva Commercial Cloud), document them:

```javascript
// Veeva CRM (Veeva Commercial Cloud) selectors (verified YYYY-MM-DD)
...
Veeva CRM (Veeva Commercial Cloud) updates its UI periodically. When selectors break:
